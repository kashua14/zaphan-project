...
function ProgramChoices({ setClearedProgrammeChoices }) {
  ...
  const [dialogTitle, setDialogTitle] = useState("Add County");
  const openAddChoice = (action, clickedChoice) => {
    setShowAddChoice(true);
    setDialogTitle(`${action} ${clickedChoice} County`);
    setChoice(action);
  };
  ...
  return (
    <Card id="programme-choices" sx={{ overflow: "visible", p: 2 }} mt={4}>
      <MDBox>
        <MDTypography variant="h5">COUNTIES</MDTypography>
      </MDBox>
      <Grid container>
