...
function ProgramChoices({ setClearedProgrammeChoices }) {
  ...
  const [dialogTitle, setDialogTitle] = useState("Add Parish");
  const openAddChoice = (action, clickedChoice) => {
    setShowAddChoice(true);
    setDialogTitle(`${action} ${clickedChoice} Parish`);
    setChoice(action);
  };
  ...
  return (
    <Card id="programme-choices" sx={{ overflow: "visible", p: 2 }} mt={4}>
      <MDBox>
        <MDTypography variant="h5">PARISHES</MDTypography>
      </MDBox>
      <Grid container>
