...
function ProgramChoices({ setClearedProgrammeChoices }) {
  ...
  const [dialogTitle, setDialogTitle] = useState("Add Sub County");
  const openAddChoice = (action, clickedChoice) => {
    setShowAddChoice(true);
    setDialogTitle(`${action} ${clickedChoice} Sub County`);
    setChoice(action);
  };
  ...
  return (
    <Card id="programme-choices" sx={{ overflow: "visible", p: 2 }} mt={4}>
      <MDBox>
        <MDTypography variant="h5">SUB COUNTIES</MDTypography>
      </MDBox>
      <Grid container>
