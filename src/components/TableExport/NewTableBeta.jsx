...
function MyComponent(props) {
  ...
  const customizeHeader = (worksheet) => {
    ...
    worksheet.getColumn(1).values = [
      "react-app:",
      "To nurture and develop technology business start-ups for job creation:",
      "Address:",
      "Web:",
    ];
  ...
  const customizeFooter = (worksheet) => {
    ...
    currentRowIndex += 1;
    worksheet.getRow(currentRowIndex).getCell(1).value = "joshua.dev";
    currentRowIndex += 1;
    worksheet.getRow(currentRowIndex).getCell(1).value = "TABLE";
    worksheet.getRow(currentRowIndex).getCell(1).font = { italic: true };
  };

  const [selection, setSelection] = useState([]);

  let fileName = "TABLE_DATA.xlsx";

  if (props.fileName !== undefined && props.fileName !== null) {
    fileName = props.fileName + "_TABLE.xlsx";
  }
