...
export default styled(TextField)(({ theme, ownerState }) => {
  ...
  const { grey, transparent, error: colorError, success: colorSuccess, background } = palette;
  const { pxToRem } = functions;

  let bgColorValue = grey[200];

  if (darkMode) {
    bgColorValue = background.default;
  }
  ...
  return {
    backgroundColor: disabled ? `${bgColorValue} !important` : transparent?.main,
